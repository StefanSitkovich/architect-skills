 ...
 * this serve-docs copy carries them all, so it can lint the whole set before
 * serving. The shared checks below are identical in every copy.
 */
...
const SCHEMAS = {
  'requirements.md': {
    kind: 'sections', level: 2,
    required: ['Vision', 'Stakeholders & Personas', 'Functional Requirements',
      'Non-Functional Requirements', 'Success Metrics',
      'Constraints & Assumptions', 'Prioritization'],
  },
  'architecture.md': {
    kind: 'sections', level: 2,
    required: ['Context', 'Modules', 'Tech Stack', 'Data Flow'],
  },
  'domainmodel.md': {
    kind: 'sections', level: 2,
    required: ['Actors', 'Core Domain', 'Relationships', 'Bounded Contexts', 'Business Rules'],
    optional: ['Glossary'],
  },
  'contracts.md': {
    // Feature-grouped, not a fixed header set — generic checks + an H1 only.
    kind: 'free',
  },
  'dir:entities': {
    kind: 'sections', level: 2,
    required: ['Description', 'Key(s)', 'Fields / Information'],
    optional: ['Status'],
  },
  'decisions.md': {
    kind: 'entries',
    entries: {
      headingLevel: 3, headingPattern: /^ADR-\d{4}\b/, example: '### ADR-0001 — Title',
      requiredFields: ['Status', 'Context', 'Decision', 'Consequences', 'Alternatives'],
    },
  },
  'roadmap.md': {
    kind: 'entries',
