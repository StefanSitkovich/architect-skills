...
1. **Gather the pages** — the `### Pages` of `Contracts.md` (routes and
   interactions), the personas and journey from `Requirements.md`, the ubiquitous
   language from `DomainModel.md`. If the pages aren't defined yet, raise that gap —
   a clickdummy of undefined pages is guesswork.
...
4. **Hand it over** — share the link / folder. The feedback is which flows feel
   right and which screens are missing — that feeds back into `Requirements.md` /
   `Contracts.md`.
...
- Click-dummying pages that were never defined — define them in `Contracts.md`
  first, or the dummy invents requirements silently.
